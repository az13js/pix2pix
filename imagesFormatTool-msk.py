...
def fileToNumpy(file, width = 0, high = 0, mod = 0):
    imageObject = Image.open(file).convert('RGB')
    if width > 0 and high > 0:
        if (width != imageObject.size[0] or high != imageObject.size[1]):
            # 计算宽度和高度各自需要的放大比例
            walpha = float(width) / imageObject.size[0]
            halpha = float(high) / imageObject.size[1]
            minAlpha = min(walpha, halpha)
            maxAlpha = max(walpha, halpha)
            if (0 == mod):
                # 计算出图片缩放后不包括边框的宽度和高度
                ...
                wImage = int(maxAlpha * imageObject.size[0])
                hImage = int(maxAlpha * imageObject.size[1])
                x = int((wImage - width) / 2)
                y = int((hImage - high) / 2)
                imageObject = imageObject.resize((wImage, hImage), Image.ANTIALIAS).crop((x, y, x + width, y + high))
...
# 返回值是转换的文件的数量
def formateImagesFrom(src, dst, width = 0, high = 0, mod = 0):
    idx = 0
    if os.path.exists(src):
        ...
        for file in fileList:
            ...
            if os.path.exists(filePath):
                if os.path.isfile(filePath) and isImage(filePath):
                    # 这里读取filePath
                    data = fileToNumpy(filePath, width, high, mod)
                    idx = idx + 1
...
deleteImages("inputs")
num = formateImagesFrom("src", "outputs", 256, 256, 1)
